...
class Common_EweiShopV2Model
{
	...
	public function getCitys($refresh = false) 
	{
		global $_W;
		$allcitys = m('cache')->getArray('allcitys');
		if (empty($allcitys) || $refresh) 
		{
			$allcitys = array();
			$citys = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_city') . ' WHERE uniacid =:uniacid AND status=1 ORDER BY id ASC, displayorder DESC', array(':uniacid' => $_W['uniacid']));
			foreach ($citys as $index => $row ) 
			{
				array_push($allcitys,$row);
			}
			m('cache')->set('allcitys', $allcitys);
		}
		return $allcitys;
	}
