...
class Shop_EweiShopV2Model 
{
	...
	public function getBrand($refresh = false) 
	{
		global $_W;
		$allbrands = m('cache')->getArray('allbrands');
		if (empty($allbrands) || $refresh) 
		{
			$allbrands = array();
			$brands = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_goods_brand') . ' WHERE uniacid =:uniacid AND status=1 ORDER BY id ASC, displayorder DESC', array(':uniacid' => $_W['uniacid']));
			foreach ($brands as $index => $row ) 
			{
				array_push($allbrands,$row);
			}
			m('cache')->set('allbrands', $allbrands);
		}
		return $allbrands;
	}
	...
	public function getExchange($refresh = false) 
	{
		global $_W;
		$exchange = m('cache')->getArray('exchange');
		if (empty($exchange) || $refresh) 
		{
			$exchange = array();
			$data = pdo_fetch('SELECT content FROM ' . tablename('ewei_shop_exchange') . ' WHERE type=1 AND status=1 ORDER BY id ASC, displayorder DESC', array(':uniacid' => $_W['uniacid']));
			$exchangeStr = $data['content'];
			if (!(empty($exchangeStr))) 
			{
				$arr1 = explode(',', $exchangeStr);
				foreach($arr1 as $key => $value){
					$arr2 = explode(':', $value);
					$exchange[] = array('0'=>$arr2[0], '1' => $arr2[1]);
				}
				asort($exchange);
			}
			m('cache')->set('exchange', $exchange);
		}
		return $exchange;
	}
}
