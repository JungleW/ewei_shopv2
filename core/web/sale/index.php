...
class Index_EweiShopV2Page extends ComWebPage 
{
	...
	public function main() 
	{
		...
		if (cv('sale.exchange')) 
		{
			header('location: ' . webUrl('sale/exchange'));
			return;
		}
		if (cv('sale.coupon')) 
		{
	...
	public function exchange() 
	{
		...
		{
			$exchangeStr = '';
			$postexchange = $_GPC['enough'];
			if($postexchange)
			{
				foreach ($postexchange as $key => $value ) 
				{
					$enough = floatval($value);
				 	if (0 < $enough) 
				 	{
				 		$exchangeStr .= ','.floatval($postexchange[$key]).':'.floatval($_GPC['give'][$key]);
				 	}
				}
				$exchangeStr = substr($exchangeStr, 1);
			}

			$d = array('content' => $exchangeStr, 'status'=>1);

			$res = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_exchange') . ' WHERE type=1');

			if (!(empty($res))) 
			{
				pdo_update('ewei_shop_exchange', $d, array('type' => 1));
				plog('sale.exchange.edit', '修改基本积分兑换规则');
			}
			else 
			{
				$d['uniacid'] = $uniacid;
				$d['type'] = 1;
				pdo_insert('ewei_shop_exchange', $d);
				plog('sale.exchange.edit', '修改基本积分兑换规则(type:1)');
			}
			show_json(1, array('url' => webUrl('sale/exchange', array('tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}
		$exchange = m('shop')->getExchange(true);
		include $this->template();
	}
}
?>