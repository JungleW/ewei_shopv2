...
class Permission_EweiShopV2Page extends WebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($item))) 
			{
				...
				plog('city.permission.edit', '修改品牌 ID: ' . $id);
			}
			else 
			{
				...
				$id = pdo_insertid();
				plog('city.permission.add', '修改品牌 ID: ' . $id);
			}
			show_json(1, array('url' => webUrl('city/permission/edit', array('id' => $id))));
		}
		include $this->template();
	}
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('city.permission.edit', (('修改品牌状态<br/>ID: ' . $item['id'] . '<br/>品牌名称: ' . $item['brand'] . '<br/>状态: ' . $_GPC['status']) == 1 ? '上架' : '下架'));
		}
		show_json(1, array('url' => referer()));
	}
