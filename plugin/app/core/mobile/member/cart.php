...
class Cart_EweiShopV2Page extends AppMobilePage
{
    public function get_cart()
    {
        ...
        $sql = 'SELECT f.id,f.total,f.goodsid,g.total as stock, o.stock as optionstock, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,' . ' g.productprice,o.title as optiontitle,f.optionid,o.specs,g.minbuy,g.maxbuy,g.maxprice, g.minprice, g.discounts, g.unit,f.merchid,g.merchsale' . ' ,f.selected FROM ' . tablename('ewei_shop_member_cart') . ' f ' . ' left join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' . ' left join ' . tablename('ewei_shop_goods_option') . ' o on f.optionid = o.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
        ...
        foreach ($list as &$g) {
            ...
            $memberprice = m('goods')->getMemberPrice($g, $level);
            if(!empty($memberprice)){
                $g['marketprice'] = $memberprice;
                $g['maxprice'] = $memberprice;
                $g['minprice'] = $memberprice;
            }
            unset($g['maxbuy']);
        }
