	...
	public function get_list()
	{
		...
		if (0 < $goods['total']) {
			...
			foreach ($goods_list as $index => $item) {
				...
				$memberprice = m('goods')->getMemberPrice($goods_list[$index], $level);

				if (!empty($memberprice) && ($memberprice!= $goods_list[$index]['minprice']) && !empty($level)) {
					$goods_list[$index]['minprice'] = $memberprice;
	...
	public function get_detail()
	{
		...
		if (empty($goods['isdiscount']) || (!empty($goods['isdiscount']) && ($goods['isdiscount_time'] < time()))) {
			if (!empty($memberprice) && ($memberprice != $goods['minprice']) && !empty($level)) {
				$goods['minprice'] = $memberprice;
				$goods['maxprice'] = $memberprice;
			}
		}
